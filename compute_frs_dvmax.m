function result_folder = compute_frs_dvmax(velocities, dvmax_values, varargin)
% COMPUTE_FRS_DVMAX Computes forward reachable sets for vehicle with different
...
%                   'uMode'     - Control mode ('max' for FRS) (default: 'max')
%
% Output:
%   result_folder - Path to the folder where results are saved
%
% Example:
%   result_folder = compute_frs_dvmax([15, 30], [deg2rad(5), deg2rad(10)], 'tMax', 0.5);
...
p.addParameter('uMode', 'max', @ischar); % Default is 'max' for forward reachability
...
folder_name = sprintf('steered_frs_results_%s', timestamp); % Changed from 'brs' to 'frs'
...
sim_params.reachability_type = 'forward'; % Added to indicate FRS
...
for v_idx = 1:length(velocities)
    ...
    for d_idx = 1:length(dvmax_values)
        ...
        % Display progress
        fprintf('Computing FRS for velocity = %d m/s, dv_max = %.2f rad/s (%.2f deg/s)...\n', ...
        ...
        if strcmp(opts.uMode, 'max')
            % For forward reachability (maximizing reachable area):
            control_grid(derivs{3} >= 0) = params(7);   % dv_max
            control_grid(derivs{3} < 0) = -params(7);   % -dv_max
        else
            % For other control objectives:
            control_grid(derivs{3} >= 0) = -params(7);  % -dv_max
            control_grid(derivs{3} < 0) = params(7);    % dv_max
        end
        ...
        result_filename = sprintf('frs_v%d_dvmax%.0f.mat', velocities(v_idx), dvmax_values(d_idx) * 180/pi);
...
save(fullfile(result_folder, 'frs_combined_results.mat'), ...
...
function derivs = extractCostates3D(g, data)
    ...
    for i = 1:g.dim
        ...
        % Choose appropriate derivative based on Hamiltonian maximization
        if i == 3  % For steering control (delta dimension)
            % For forward reachability (maximizing), use the derivative that gives the larger value
            derivs{i} = deriv_right;
            derivs{i}(deriv_left > deriv_right) = deriv_left(deriv_left > deriv_right);
        else
