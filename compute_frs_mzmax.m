function result_folder = compute_frs_mzmax(velocities, mzmax_values, varargin)
% COMPUTE_FRS_MZMAX Computes forward reachable sets for vehicle with different
...
%
% Example:
%   result_folder = compute_frs_mzmax([15, 30], [5000, 10000], 'tMax', 0.5);
...
folder_name = sprintf('frs_results_%s', timestamp); % Changed from 'brs' to 'frs'
...
for v_idx = 1:length(velocities)
    ...
    for m_idx = 1:length(mzmax_values)
        ...
        % Display progress
        fprintf('Computing FRS for velocity = %d m/s, Mzmax = %d N·m...\n', ...
        ...
        result_filename = sprintf('frs_v%d_mz%d.mat', velocities(v_idx), mzmax_values(m_idx));
