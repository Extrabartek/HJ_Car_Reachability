...
if opts.visualize
    ...
    if is_3d_model
        % For 3D model, create slice visualizations
        visualize_3d_safe_set(g, safe_set_value_function, brs_value_function, frs_value_function, velocity, control_limit, control_type);
    else
        % For 2D model
...
function visualize_3d_safe_set(g, safe_set, brs, frs, velocity, control_limit, control_type)
    ...
    num_slices = 3;
    slice_indices = round(linspace(1, length(delta_values), num_slices));
    ...
    if strcmp(control_type, 'dv')
        sgtitle(sprintf('Safe Set Slices (v = %d m/s, dvmax = %.1f°/s)', ...
                velocity, control_limit*180/pi), 'FontSize', 16, 'FontWeight', 'bold');
    else
