function visualize_frs_results(result_folder, varargin)
% VISUALIZE_FRS_RESULTS Creates visualizations from saved FRS computation results
%
% Inputs:
%   result_folder - Path to the folder containing FRS computation results
%   varargin      - Optional parameter-value pairs:
%                   'plotType'   - Types of plots to generate (cell array)
%                                 Options: 'control', 'detailed', 'comparison', 'velocity_stack'
%                                 (default: {'control', 'detailed'})
%                   'saveFigs'   - Whether to save figures (default: true)
%                   'figFormat'  - Format to save figures in (default: 'png')
%
% Example:
%   visualize_frs_results('frs_results_20250325_123456_vx30-50_mz5000-10000');
%   visualize_frs_results('frs_results_folder', 'plotType', {'comparison'});
    ...
    function generate_control_plots()
        ...
        for v_idx = 1:length(velocities)
            for m_idx = 1:length(mzmax_values)
                ...
                % Get the FRS data and control data for this combination
                current_frs = all_data{v_idx, m_idx};
                ...
                % Plot the FRS boundary
                [~, h_frs] = contour(xs1_deg, xs2_deg, current_frs, [0 0], 'LineWidth', 2, 'Color', 'k');
                ...
                title(sprintf('FRS and Optimal Control for v = %d m/s, Mzmax = %d N·m', ...
                      velocities(v_idx), mzmax_values(m_idx)), 'FontSize', 14, 'FontWeight', 'bold');
                legend([h_frs, h_target], 'FRS Boundary', 'Target Set', 'Location', 'best', 'FontSize', 10);
    ...
    function generate_detailed_plots()
        ...
        for v_idx = 1:length(velocities)
            for m_idx = 1:length(mzmax_values)
                ...
                derivs = extractCostates(g, current_frs);
                ...
                surf(xs1_deg, xs2_deg, current_frs, 'EdgeColor', 'none');
                colormap(gca, parula);
                ...
                hold on;
                [~, h_frs] = contour3(xs1_deg, xs2_deg, current_frs, [0 0], 'LineWidth', 2, 'Color', 'r');
...
%% Nested function to generate 3D plots showing FRSs for different speeds
    function generate_3d_velocity_stack()
        ...
        if length(velocities) > 1
            % Compare FRS for different velocities with the same Mzmax
            for m_idx = 1:length(mzmax_values)
                ...
                for v_idx = 1:length(velocities)
                    [~, h_frs(v_idx)] = contour3(xs1_deg, xs2_deg, ...
    ...
    function generate_comparison_plots()
        ...
        if length(velocities) > 1
            ...
            for m_idx = 1:length(mzmax_values)
                ...
                h_frs = zeros(length(velocities), 1);
                for v_idx = 1:length(velocities)
                    [~, h_frs(v_idx)] = contour(xs1_deg, xs2_deg, all_data{v_idx, m_idx}, [0 0], ...
                        'LineWidth', 2, 'Color', vel_colors(v_idx,:));
                end
        ...
        if length(mzmax_values) > 1
            % Compare FRS for different Mzmax with the same velocity
            for v_idx = 1:length(velocities)
                ...
                % Plot FRS boundary for each Mzmax
                h_frs = zeros(length(mzmax_values), 1);
                for m_idx = 1:length(mzmax_values)
                    [~, h_frs(m_idx)] = contour(xs1_deg, xs2_deg, all_data{v_idx, m_idx}, [0 0], ...
                ...
                xlabel('Sideslip Angle (degrees)', 'FontSize', 12);
                ylabel('Yaw Rate (degrees/s)', 'FontSize', 12);
                title(sprintf('FRS Comparison for Different Yaw Moment Limits (v = %d m/s)', ...
        ...
        % Show how the FRS evolves over time

        for v_idx=1:length(velocities)
            for m_idx=1:length(mzmax_values)
                
                if ~isempty(all_data_full) && ~isempty(all_data_full{v_idx, m_idx})
                    figure('Name', 'FRS Time Evolution');
                    clf;
                    ...
                    % Plot FRS boundary for each time point
                    ...
                    xlabel('Sideslip Angle (degrees)', 'FontSize', 12);
                    ylabel('Yaw Rate (degrees/s)', 'FontSize', 12);
                    title(sprintf('FRS Time Evolution (v = %d m/s, Mzmax = %d N·m)', ...
                          velocities(v_idx), mzmax_values(m_idx)), 'FontSize', 14, 'FontWeight', 'bold');
...
function derivs = extractCostates(g, data)
    ...
    for i = 1:g.dim
        ...
        % For forward reachability, we generally want to maximize the Hamiltonian
        if i == 1  % beta dimension
            % Use central differencing for this dimension
