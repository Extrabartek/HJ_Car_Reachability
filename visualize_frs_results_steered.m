function visualize_frs_results_steered(result_folder, varargin)
% VISUALIZE_FRS_RESULTS_STEERED Creates visualizations from saved FRS computation results
% for the 3-state NonlinearBicycleSteered model
%
% Inputs:
%   result_folder - Path to the folder containing FRS computation results
...
%
% Example:
%   visualize_frs_results_steered('steered_frs_results_20250325_123456_vx30-50_dvmax5-10');
%   visualize_frs_results_steered('results_folder', 'plotType', {'slices'}, 'deltaSlices', [-0.05, 0, 0.05]);
...
if ~exist(combined_file, 'file')
    % Try alternative filename format for backward compatibility
...
%% Nested function to generate 2D slices of 3D FRS at different steering angles
    function generate_slice_plots()
        ...
        for v_idx = 1:length(velocities)
            for d_idx = 1:length(dvmax_values)
                ...
                current_frs = all_data{v_idx, d_idx};
                current_control = control_data{v_idx, d_idx};
                
                % Create a figure for this combination
                figure('Name', sprintf('FRS Slices - v=%d, dvmax=%.1f°/s', ...
                ...
                for s = 1:length(slice_indices)
                    ...
                    % Extract 2D slice
                    frs_slice = squeeze(current_frs(:,:,slice_idx));
                    ...
                    % Plot FRS boundary - sideslip on x, yaw rate on y
                    [~, h_frs] = contour(xs2_deg, xs1_deg, frs_slice, [0 0], 'LineWidth', 2, 'Color', 'k');
                    ...
                    legend([h_frs, h_target], 'FRS Boundary', 'Target Set', 'Location', 'best');
                ...
                sgtitle(sprintf('FRS Slices for v = %d m/s, Max Steering Rate = %.1f°/s', ...
    ...
    function generate_detailed_plots()
        ...
        for v_idx = 1:length(velocities)
            for d_idx = 1:length(dvmax_values)
                ...
                frs_slice = squeeze(current_frs(:,:,center_slice_idx));
                target_slice = squeeze(data0(:,:,center_slice_idx));
                ...
                % 2. FRS Boundary with Control Switching Surface
                subplot(2, 2, 2);
                ...
                % For 3D FRS, need to extract 2D derivatives
                derivs = extract2DCostates(g, frs_slice);
                ...
                title('FRS Boundary and Control Switching Surface', 'FontSize', 12);
                xlabel('Sideslip Angle (deg)', 'FontSize', 10);
                ylabel('Yaw Rate (deg/s)', 'FontSize', 10);
                legend([h_frs, h_target, h_switch], 'FRS Boundary', 'Target Set', 'Control Switching', 'Location', 'best', 'FontSize', 8);
                xlim(x_limits);
                ylim(y_limits);
                grid on;
                ...
                surf(xs2_deg, xs1_deg, frs_slice, 'EdgeColor', 'none');
                colormap(gca, parula);
                
                % Adjust view angle
                view([-30, 30]);
                
                % Add the zero level set (FRS boundary)
                hold on;
                [~, h_frs] = contour3(xs2_deg, xs1_deg, frs_slice, [0 0], 'LineWidth', 2, 'Color', 'r');
...
%% Nested function to generate comparison plots across different parameters
    function generate_comparison_plots()
        ...
        if length(dvmax_values) > 1
            % Compare FRS for different steering rates with the same velocity
            for v_idx = 1:length(velocities)
                ...
                % Plot FRS boundary for each steering rate
                h_frs = zeros(length(dvmax_values), 1);
                for d_idx = 1:length(dvmax_values)
                    % Extract FRS slice
                    frs_slice = squeeze(all_data{v_idx, d_idx}(:,:,center_slice_idx));
                    
                    % Sideslip on x, yaw rate on y
                    [~, h_frs(d_idx)] = contour(xs2_deg, xs1_deg, frs_slice, [0 0], ...
                ...
                title(sprintf('FRS Comparison for Different Steering Rate Limits (v = %d m/s, δ = %.1f°)', ...
                      velocities(v_idx), delta_val*180/pi), 'FontSize', 14, 'FontWeight', 'bold');
                legend([h_frs; h_target], legend_entries, 'Location', 'best', 'FontSize', 10);
        ...
        if length(velocities) > 1
            % Compare FRS for different velocities with the same steering rate
            for d_idx = 1:length(dvmax_values)
                ...
                % Plot FRS boundary for each velocity
                h_frs = zeros(length(velocities), 1);
                for v_idx = 1:length(velocities)
                    % Extract FRS slice
                    frs_slice = squeeze(all_data{v_idx, d_idx}(:,:,center_slice_idx));
                    
                    % Sideslip on x, yaw rate on y
                    [~, h_frs(v_idx)] = contour(xs2_deg, xs1_deg, frs_slice, [0 0], ...
                ...
                xlabel('Sideslip Angle (degrees)', 'FontSize', 12);
                ylabel('Yaw Rate (degrees/s)', 'FontSize', 12);
                title(sprintf('FRS Comparison for Different Velocities (Max Steering Rate = %.1f°/s, δ = %.1f°)', ...
        ...
        for v_idx = 1:length(velocities)
            for d_idx = 1:length(dvmax_values)
                ...
                % Plot FRS boundary for each delta slice
                h_frs = zeros(length(delta_indices), 1);
                for s = 1:length(delta_indices)
                    ...
                    % Sideslip on x, yaw rate on y
                    [~, h_frs(s)] = contour(xs2_deg, xs1_deg, frs_slice, [0 0], ...
                ...
                xlabel('Sideslip Angle (degrees)', 'FontSize', 12);
                ylabel('Yaw Rate (degrees/s)', 'FontSize', 12);
                title(sprintf('FRS Comparison for Different Steering Angles (v = %d m/s, Max Steering Rate = %.1f°/s)', ...
